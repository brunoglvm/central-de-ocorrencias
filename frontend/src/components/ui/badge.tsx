...
const badgeVariants: Record<IncidentStatus, string> = {
  ...
  in_progress: 'bg-[var(--badge-progress-bg)] text-[var(--badge-progress-text)]',
...
export function Badge({ children, status }: BadgeProps) {
  return (
    <span
      className={cn('inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium', badgeVariants[status])}
    >
      {children}
    </span>
  )
