...
export const registerIncidentSchema = z
  .object({
    ...
    userType: z.string().min(1, 'Selecione quem esta registrando.'),
  })
  .superRefine((values, context) => {
    if (values.location === CUSTOM_LOCATION_VALUE && values.customLocation.trim().length < 3) {
      context.addIssue({
